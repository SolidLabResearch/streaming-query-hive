 ...
 * Tests only exponential growth/decay patterns with different rates using fetching client side approach
 */
...
const LOGS_DIR = 'logs/rate-comparison-fetching';
...
async function runSingleTest(rate, pattern, iteration = 1) {
  ...
  console.log(`\n🧪 Testing Fetching Client Side: ${runLabel}`);
  ...
  try {
    ...
    console.log(`⏱️  ${runLabel} completed in ${duration.toFixed(1)}s`);
...
async function runAllRateTests() {
  console.log('Starting Exponential Rate Comparison - Fetching Client Side Approach');
...
async function runSpecificRate(rate) {
  ...
  console.log(`Testing Fetching Client Side Approach for rate: ${rate}`);
...
async function runSpecificTest(pattern, rate) {
  ...
  console.log(`Testing Fetching Client Side Approach: ${pattern} with rate ${rate}`);
...
function generateSummaryReport(results) {
  console.log('\n' + '='.repeat(80));
  console.log('FETCHING CLIENT SIDE APPROACH - RATE COMPARISON SUMMARY');
  ...
  const reportPath = path.join(LOGS_DIR, 'fetching_rate_comparison_summary.json');
...
async function main() {
  ...
  try {
    if (args.length === 0) {
      ...
      console.log('Usage:');
      console.log('  node experiment-rate-comparison-fetching.js                           # Run all rate tests');
      console.log('  node experiment-rate-comparison-fetching.js rate 0.1                 # Run tests for specific rate');
      console.log('  node experiment-rate-comparison-fetching.js test exponential_growth 1 # Run specific test');
      process.exit(1);
    }
  } catch (error) {
