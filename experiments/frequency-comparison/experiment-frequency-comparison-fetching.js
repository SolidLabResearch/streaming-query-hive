...
/**
 * Frequency Comparison Experiment - Fetching Client Side Approach
 * Tests fetching accuracy across different oscillation frequencies
 */
...
class FrequencyComparisonFetchingExperiment {
    constructor() {
    ...
    async runSingleTest(oscillationType, frequency) {
        // Format frequency consistently (ensure decimal notation for non-integers)
        ...
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                ...
                publisher.on('close', (code) => {
                    ...
                    const fetchingResourceFile = path.join(testLogDir, 'fetching_client_side_resource_usage.csv');
                    const replayerLogFile = path.join(testLogDir, 'replayer-log.csv');
                    
                    fs.writeFileSync(fetchingLogFile, approachStdout);
                    fs.writeFileSync(fetchingResourceFile, approachStderr);
...
if (process.argv.length > 2) {
    ...
    if (command === 'test' && process.argv.length >= 5) {
        ...
        const frequency = parseFloat(process.argv[4]);
        
        const experiment = new FrequencyComparisonFetchingExperiment();
        experiment.runSingleTest(oscillationType, frequency)
            .then(result => {
        ...
        console.log('Usage:');
        console.log('  node experiment-frequency-comparison-fetching.js                    # Run all tests');
        console.log('  node experiment-frequency-comparison-fetching.js test simple_oscillation 1.0  # Run specific test');
        process.exit(1);
    }
} else {
    // Run all tests
