...
function compareApproaches() {
    ...
    if (approxMemory && chunkedMemory) {
        ...
        if (memoryImprovement > 0) {
            ...
            console.log(`\n  Memory Usage: ${Math.abs(memoryImprovement).toFixed(1)}% increase vs chunked approach`);
        }
    }
    ...
    if (approxMemTrend && chunkedMemTrend) {
        ...
        if (approxMemTrend.growthRate === 'Increasing' && chunkedMemTrend.growthRate === 'Decreasing') {
            console.log('  Approximation shows memory growth, chunked approach is more stable');
    ...
    if (approxMemory && chunkedMemory) {
        ...
        if (avgDiff < 0) {
            ...
            console.log(`     Approximation uses ${avgDiff.toFixed(1)}% more memory on average`);
        }
        
        if (peakDiff < 0) {
            console.log(`    Peak memory usage ${Math.abs(peakDiff).toFixed(1)}% lower`);
        } else {
            console.log(`     Peak memory usage ${peakDiff.toFixed(1)}% higher`);
        }
    }
